...
import { Sparkles, BrainCircuit, Quote, Zap, Loader2, ChevronDown, ChevronUp } from 'lucide-react';
...
const PepeOracle: React.FC<PepeOracleProps> = ({ data }) => {
  ...
  const [isExpanded, setIsExpanded] = useState(false); // Estado para minimizar
    ...
    <div className="w-full max-w-4xl mx-auto mb-16 px-4">
      ...
      {!isExpanded ? (
        <button 
            ...
            className="w-full group relative overflow-hidden rounded-2xl bg-slate-900/80 backdrop-blur-md border border-indigo-500/30 p-4 transition-all duration-300 hover:bg-slate-800/80 hover:border-indigo-500/60 shadow-[0_0_20px_rgba(99,102,241,0.1)] flex items-center justify-between"
        >
             ...
             <div className="flex items-center gap-4 relative z-10">
                 <div className="w-10 h-10 rounded-full bg-slate-800 border border-indigo-500/40 flex items-center justify-center overflow-hidden shrink-0">
                     <img src={PEPE_ASSETS.ORACLE_AVATAR} className="w-full h-full object-cover grayscale opacity-70 group-hover:grayscale-0 group-hover:opacity-100 transition-all" alt="Oracle" />
                 </div>
                 <div className="text-left">
                     ...
                     <p className="text-[10px] text-slate-400 font-medium">Click para invocar una predicción mística</p>
                 </div>
             </div>

             <div className="p-2 bg-indigo-500/10 rounded-full text-indigo-400 relative z-10 group-hover:bg-indigo-500 group-hover:text-white transition-all">
                 <ChevronDown size={20} />
             </div>
        </button>
      ) : (
        ...
        <div className={`
            bg-slate-950/80 backdrop-blur-3xl border border-indigo-500/30 rounded-[4rem] p-10 md:p-16 shadow-[0_0_80px_rgba(99,102,241,0.1)] relative overflow-hidden group transition-all duration-1000 animate-in fade-in slide-in-from-top-4
        ...
        `}>
            ...
            <button 
                onClick={toggleExpand}
                className="absolute top-6 right-6 p-2 rounded-full bg-slate-900/50 text-slate-500 hover:text-white hover:bg-slate-800 transition-all z-40 border border-slate-700/50"
            >
                <ChevronUp size={24} />
            </button>
